...
async fn test_create_contact(pool: PgPool) -> sqlx::Result<()> {
    ...
    let found = repo.find_by_owner_and_phone(&owner_id, &phone).await.expect("Should find contact");
...
async fn test_find_all_by_owner(pool: PgPool) -> sqlx::Result<()> {
    ...
    repo.create(&contact1).await.expect("Should create contact1");
    repo.create(&contact2).await.expect("Should create contact2");

    let all = repo.find_all_by_owner(&owner_id).await.expect("Should find all contacts");
    assert_eq!(all.len(), 2);
...
async fn test_update_contact(pool: PgPool) -> sqlx::Result<()> {
    ...
    let mut found = repo.find_by_owner_and_phone(&owner_id, &phone).await.expect("Should find").unwrap();
    found.set_favorite(true);
    repo.update(&found).await.expect("Should update");

    let updated = repo.find_by_owner_and_phone(&owner_id, &phone).await.expect("Should find").unwrap();
...
async fn test_delete_contact(pool: PgPool) -> sqlx::Result<()> {
    ...
    let found = repo.find_by_owner_and_phone(&owner_id, &phone).await.expect("Should find").unwrap();
